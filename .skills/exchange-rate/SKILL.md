...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-9wmtpvxmtm9t-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/{base_currency}` |
...
async function getExchangeRates(baseCurrency: string): Promise<ExchangeRateResponse> {
  ...
  const url = `https://app-9wmtpvxmtm9t-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/${baseCurrency.toUpperCase()}`;
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-9wmtpvxmtm9t-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/${baseCurrency.toUpperCase()}`,
    {
      method: "GET",
      headers: {
        "Accept": "application/json",
