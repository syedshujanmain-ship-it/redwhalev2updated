...
export type TypingIndicatorStyle = 'dots' | 'wave' | 'pulse' | 'orbit' | 'neon' | 'matrix' | 'fire' | 'heart' | 'galaxy' | 'rocket' | 'crown' | 'lightning' | 'ocean' | 'diamond' | 'box_up' | 'box_down' | 'box_bounce' | 'square';
...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  customModes: [],
  cursorStyle: 'beam',
  typingIndicatorStyle: 'square',
  thinkingMode: false,
  redGlowEnabled: false,
...
export function useAppSettings() {
  ...
  const resetSettings = useCallback(() => {
    ...
    try {
      ...
      localStorage.removeItem('redwhale_app_settings');
      localStorage.removeItem('rw_language');
    } catch {
      // ignore
    }
    // Reload to apply font and DPI changes cleanly
